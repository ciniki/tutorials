...
function ciniki_tutorials_settingsUpdate(&$ciniki) {
    ...
    $rc = ciniki_tutorials_checkAccess($ciniki, $args['tnid'], 'ciniki.tutorials.settingsUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    $rc = ciniki_core_dbDetailsQuery($ciniki, 'ciniki_tutorial_settings', 'tnid', $args['tnid'], 'ciniki.tutorials', 'settings', '');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    foreach($changelog_fields as $field) {
        if( isset($ciniki['request']['args'][$field]) 
            ...
            $strsql = "INSERT INTO ciniki_tutorial_settings (tnid, detail_key, detail_value, date_added, last_updated) "
            ...
            ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.tutorials', 'ciniki_tutorial_history', $args['tnid'], 
